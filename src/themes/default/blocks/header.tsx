...
const buildChildGroups = (children?: NavItem[]): NavChildGroup[] => {
  if (!children) return [];
  return children
    .map((child) => {
      const layout = (child.layout as 'single' | 'double') || 'double';
      if (child.items?.length) {
        return {
          dropdown_title: child.dropdown_title ?? child.title,
          items: child.items,
          layout,
        };
      }
      return {
        dropdown_title: child.dropdown_title,
        items: [child],
        layout,
      };
    })
...
export function Header({ header }: { header: HeaderType }) {
  ...
  const navBadgeClass =
    'bg-gradient-to-r from-[#FCA5F1] via-[#FFD48B] to-[#FFB347] text-[10px] font-semibold uppercase tracking-wide text-black/80 px-2 py-0.5 rounded-full leading-none shadow-sm border border-white/30 whitespace-nowrap';
              ...
              {item.children && item.children.length > 0 ? (
                <>
                  ...
                  <NavigationMenuContent className="mt-4.5 origin-top pt-5 pb-14 shadow-none ring-0">
                    <div className="divide-foreground/10 w/full grid min-w-6xl grid-cols-4 gap-4 divide-x pr-22">
                      {buildChildGroups(item.children).map(
                        (group, groupIdx) => {
                          const spanClass =
                            group.layout === 'single'
                              ? 'col-span-4'
                          ...
                          return (
                            ...
                            >
                              {group.dropdown_title ? (
                                <span className="text-muted-foreground ml-2 text-xs">
                                  {group.dropdown_title}
                                </span>
                              ) : null}
                              <ul className="mt-1 grid grid-cols-2 gap-2">
                                {group.items.map((subItem: NavItem, iidx) => (
                                  <ListItem
                                    key={iidx}
                                    href={subItem.url || ''}
                                    title={subItem.title || ''}
          ...
          <Link href={href} className="grid grid-cols-[auto_1fr] gap-3.5">
            ...
            <div className="space-y-0.5">
              <div className="flex items-start justify-between gap-2">
                ...
                {badge ? (
                  <Badge className={`${navBadgeClass} shrink-0`}>{badge}</Badge>
                ) : null}
              </div>
